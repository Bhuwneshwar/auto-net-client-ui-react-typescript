...
const Signup = () => {
  ...
  const {
    dispatch,
    store: { successResponseData, MyDetails },
  } = useGlobalContext();
  ...
  useEffect(() => {
    try {
      if (successResponseData) {
        ...
        console.log({ objData });
        if (objData.type === "new-user-register") {
          toast.success("Registration successful", {
            position: "bottom-center",
          });
          if (MyDetails) {
            dispatch("MyDetails", { ...MyDetails, Balance: objData.Balance });
          }
          genPdf(objData.username);
        }
      }
    } catch (error) {
      console.log(error);
      ...
      <div className="signup-form">
        <form onSubmit={(e) => e.preventDefault()}>
          <div className="bg-white">
            <h1>{MyDetails ? "Add New User" : "Sign-up Form"}</h1>
            <div className="biog">
